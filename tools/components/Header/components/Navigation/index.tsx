...
export interface NavigationState {navActive: boolean}
...
export default class Navigation extends React.Component<NavigationProps, NavigationState> {
  ...
  static items: any = [
    {name: 'Source Code', rel: false, url: `${Navigation.projectGithubLink}`},
    {name: 'Issues', rel: false, url: `${Navigation.projectGithubLink}/issues`},
    {
      name: 'Pull request',
      rel: false,
  ...
  toggleMobileNavigation() {
    this.setState({...this.state, navActive: !this.state.navActive});
  }
        ...
        <div className={style.header__nav__items}>
          {
            Navigation.items.map(
              (item: ItemProps) => <Item key={item.url} {...item} />,
            )
          }
        </div>
        <div
          className={style.header__nav__mobileMenuBtn}
